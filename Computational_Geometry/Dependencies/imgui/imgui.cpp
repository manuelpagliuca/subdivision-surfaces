...
/*
 ...
 EXHIBIT 2: IMPLEMENTING CUSTOM BINDING / CUSTOM ENGINE
	 ...
	 io.Fonts->GetTexDataAsRGBA32(&pixels, &m_width, &m_height);

	 // At this point you've got the texture data and you need to upload that your your graphic system:
	 // After we have created the texture, store its pointer/identifier (_in whichever format your engine uses_) in 'io.Fonts->TexID'.
	 // This will be passed back to your via the renderer. Basically ImTextureID == void*. Read FAQ for details about ImTextureID.
	 MyTexture* texture = MyEngine::CreateTextureFromMemoryPixels(pixels, m_width, m_height, TEXTURE_TYPE_RGBA32)
	 ...
	 while (true)
	 {
		...
		io.DeltaTime = 1.0f/60.0f;              // set the time rawElapsed since the previous frame (in seconds)
		io.DisplaySize.x = 1920.0f;             // set the current display m_width
		io.DisplaySize.y = 1280.0f;             // set the current display m_height here
 ...
 The bindings in impl_impl_XXX.cpp files contains many working implementations of a rendering function.
	...
	void void MyImGuiRenderFunction(ImDrawData* draw_data)
	{
	   ...
	   // TODO: Setup orthographic m_projection matrix cover draw_data->DisplayPos to draw_data->DisplayPos + draw_data->DisplaySize
	   ...
	   for (int n = 0; n < draw_data->CmdListsCount; n++)
	   {
		  ...
		  for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
		  {
			 ...
			 else
			 {
				 ...
				 // By default the m_indices ImDrawIdx are 16-bit, you can change them to 32-bit in imconfig.h if your engine doesn't support 16-bit m_indices.
 ...
 - 2019/10/14 (1.74) - inputs: Fixed a miscalculation in the keyboard/mouse "typematic" repeat delay/rate calculation, used by m_keys and e.g. repeating mouse buttons as well as the GetKeyPressedAmount() function.
 ...
 - 2016/01/23 (1.48) - fixed not honoring exact m_width passed to PushItemWidth(), previously it would add extra FramePadding.x*2 over that m_width. if you had manual pixel-perfect alignment in place it might affect you.
 ...
 - 2015/07/08 (1.43) - switched rendering data to use indexed rendering. this is saving a fair amount of CPU/GPU and enables us to get anti-aliasing for a marginal cost.
					 ...
					 - each ImDrawList now contains both a vertex buffer and an index buffer. For each command, render ElemCount/3 triangles using m_indices from the index buffer.
 ...
 - 2015/01/11 (1.30) - removed GetDefaultFontData(). uses io.Fonts->GetTextureData*() API to retrieve uncompressed pixels.
					   ...
					   - new:  unsigned char* pixels; int m_width, m_height; io.Fonts->GetTexDataAsRGBA32(&pixels, &m_width, &m_height); [..Upload texture to GPU..]; io.Fonts->TexId = YourTexIdentifier;
 ...
 - 2015/01/11 (1.30) - removed IO.PixelCenterOffset (unnecessary, can be handled in user m_projection matrix)
...
ImGuiStyle::ImGuiStyle()
{
	...
	GrabMinSize = 10.0f;            // Minimum m_width/m_height of a grab box for slider/scrollbar
	GrabRounding = 0.0f;             // Radius of grabs corners rounding. Set to 0.0f to have rectangular slider grabs.
	LogSliderDeadzone = 4.0f;             // The size in pixels of the dead-zone around zero on logarithmic sliders that cross zero.
	TabRounding = 4.0f;             // Radius of upper corners of a tab. Set to 0.0f to have rectangular tabs.
	TabBorderSize = 0.0f;             // Thickness of border around tabs.
	TabMinWidthForCloseButton = 0.0f;           // Minimum m_width for close button to appears on an unselected tab when hovered. Set to 0.0f to always show when hovering, set to FLT_MAX to never show close button unless selected.
...
bool ImGuiTextFilter::Draw(const char* label, float m_width)
{
	if (m_width != 0.0f)
...
bool ImGuiListClipper::Step()
{
	...
	// Step 0: Let you process the first element (regardless of it being visible or not, so we can measure the element m_height)
	if (StepNo == 0)
	{
		StartPosY = window->DC.CursorPos.y;
		if (ItemsHeight <= 0.0f)
		{
			// Submit the first item so we can measure its m_height (generally it is 0..1)
			DisplayStart = 0;
			DisplayEnd = 1;
			StepNo = 1;
			return true;
		...
		// Already has item m_height (given by user in Begin): skip to calculating step
		DisplayStart = DisplayEnd;
		StepNo = 2;
	}

	// Step 1: the clipper infer m_height from first element
	if (StepNo == 1)
	{
...
void ImGui::RenderTextEllipsis(ImDrawList* draw_list, const ImVec2& pos_min, const ImVec2& pos_max, float clip_max_x, float ellipsis_max_x, const char* text, const char* text_end_full, const ImVec2* text_size_if_known)
{
	...
	if (text_size.x > pos_max.x - pos_min.x)
	{
		...
		float ellipsis_total_width = ellipsis_glyph_width;      // Full m_width of entire ellipsis
...
void ImGui::SetActiveID(ImGuiID id, ImGuiWindow* window)
{
	...
	// (Please note that this is WIP and not all m_keys/inputs are thoroughly declared by all widgets yet)
...
// Note: moving window with the navigation m_keys (Square + d-pad / CTRL+TAB + Arrows) are processed in NavUpdateWindowing()
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list)
{
	...
	// - If you want large m_meshes with more than 64K m_vertices, you can either:
	//   (A) Handle the ImDrawCmd::VtxOffset value in your renderer back-end, and set 'io.BackendFlags |= ImGuiBackendFlags_RendererHasVtxOffset'.
	//       Most example back-ends already support this from 1.71. Pre-1.71 back-ends won't.
	//       Some graphics API such as GL ES 1/2 don't have a way to offset the starting vertex so it is not supported for them.
	//   (B) Or handle 32-bit m_indices in your renderer back-end, and uncomment '#define ImDrawIdx unsigned int' line in imconfig.h.
	//       Most example back-ends already support this. For example, the OpenGL example code detect index size at compile-time:
	//         glDrawElements(GL_TRIANGLES, (GLsizei)pcmd->ElemCount, sizeof(ImDrawIdx) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, idx_buffer_offset);
	//       Your own engine or render API may use different parameters or function calls to specify index sizes.
	//       2 and 4 bytes m_indices are generally supported by most graphics API.
...
// Use your own m_indices/enums according to how your back-end/engine stored them into io.KeysDown[]!
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame)
	{
		...
		else if (window->AutoFitFramesX > 0 || window->AutoFitFramesY > 0)
		{
			...
			// We still process initial auto-fit on collapsed windows to get a window m_width, but otherwise don't honor ImGuiWindowFlags_AlwaysAutoResize when collapsed.
		...
		// Default item m_width. Make it proportional to window size if window manually resizes
...
static void ImGui::ErrorCheckNewFrameSanityChecks()
{
	...
	// Perform simple check: required key mapping (we intentionally do NOT check all m_keys to not pressure user into setting up everything, but Space is required and was only recently added in 1.60 WIP)
...
void ImGui::ItemSize(const ImVec2& size, float text_baseline_y)
{
	...
	// We increase the m_height in this function to accommodate for baseline offset.
	// In theory we should be offsetting the starting position (window->DC.CursorPos), that will be the topic of a larger refactor,
	// but since ItemSize() is not yet an API that moves the cursor (to handle e.g. wrapping) enlarging the m_height has the same effect.
...
// Calculate default item m_width given value passed to PushItemWidth() or SetNextItemWidth().
...
// [Internal] Calculate full item size given user provided 'size' parameter and default m_width/m_height. Default m_width is often == CalcItemWidth().
...
ImVec2 ImGui::FindBestWindowPosForPopupEx(const ImVec2& ref_pos, const ImVec2& size, ImGuiDir* last_dir, const ImRect& r_outer, const ImRect& r_avoid, ImGuiPopupPositionPolicy policy)
{
	...
	if (policy == ImGuiPopupPositionPolicy_Tooltip || policy == ImGuiPopupPositionPolicy_Default)
	{
		...
		for (int n = (*last_dir != ImGuiDir_None) ? -1 : 0; n < ImGuiDir_COUNT; n++)
		{
			...
			// If there not enough room on one axis, there's no point in positioning on a side on this axis (e.g. when not enough m_width, use a top/bottom position to maximize available m_width)
...
static bool ImGui::NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
	...
	const ImRect& curr = g.NavScoringRect; // Current modified source rect (NB: we've applied Max.x = Min.x in NavUpdate() to inhibit the effect of having varied item m_width)
	g.NavScoringCount++;
...
#undef NAV_MAP_KEY
	...
	// FIXME-NAV: Consider enabling those m_keys even without the master ImGuiConfigFlags_NavEnableKeyboard flag?
	...
	{
		// *Fallback* manual-scroll with Nav directional m_keys when window has no navigable item
		ImGuiWindow* window = g.NavWindow;
		...
		// *Normal* Manual scroll with NavScrollXXX m_keys
...
// Handle PageUp/PageDown/Home/End m_keys
...
static void ImGui::NavUpdateWindowing()
{
	...
	// FIXME: We lack an explicit IO variable for "is the imgui window focused", so compare mouse validity to detect the common case of back-end clearing releases all m_keys on ALT-TAB
